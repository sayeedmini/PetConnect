import { useEffect, useMemo, useState } from 'react';import { Link } from 'react-router-dom';import SiteLayout from '../../../components/SiteLayout';import { getUser } from '../../auth/utils/auth';import { getAllVets } from '../services/vetApi';

const defaultFilters = {search: '',service: '',minRating: '',lat: '',lng: '',radiusKm: '',};

const DEFAULT_CLINIC_IMAGE = '/clinic-default.svg';

const serviceOptions = [{ value: '', label: 'All Services' },{ value: 'General Checkup', label: 'General Checkup' },{ value: 'Vaccination', label: 'Vaccination' },{ value: 'Surgery', label: 'Surgery' },{ value: 'Dental', label: 'Dental Care' },{ value: 'Emergency', label: 'Emergency' },];

function VetListPage() {const [vets, setVets] = useState([]);const [filters, setFilters] = useState(defaultFilters);const [loading, setLoading] = useState(true);const [sortBy, setSortBy] = useState('recommended');const currentUser = getUser();

const fetchVets = async (activeFilters = {}) => {try {setLoading(true);const cleanFilters = Object.fromEntries(Object.entries(activeFilters).filter(([, value]) => value !== '' && value !== null && value !== undefined));const data = await getAllVets(cleanFilters);setVets(data.data || []);} catch (error) {console.error(error);} finally {setLoading(false);}};

useEffect(() => {fetchVets();}, []);

const normalizedVets = useMemo(() =>vets.map((vet, index) => {const clinic = vet?.clinic || vet || {};const clinicId = vet?._id || clinic?._id || clinic-${index};const services = clinic.servicesOffered || [];const ratingValue =typeof clinic.averageRating === 'number'? clinic.averageRating: typeof clinic.rating === 'number'? clinic.rating: 0;const totalReviews = Number(clinic.totalReviews || 0);const hasReviews = totalReviews > 0;const isOpen = Boolean(vet?.isCurrentlyOpen || clinic?.isOpenNow);
...
const sortedVets = useMemo(() => {const next = [...normalizedVets];
...
const handleSubmit = (e) => {e.preventDefault();fetchVets(filters);};

const handleReset = () => {setFilters(defaultFilters);fetchVets(defaultFilters);};

const formatFee = (fee) => (fee === null || fee === undefined ? 'BDT N/A' : BDT ${fee});

return (Find Veterinary ClinicsDiscover top-rated veterinary care near you.
